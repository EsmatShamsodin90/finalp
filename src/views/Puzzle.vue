...
export default {
  ...
  if (annyang) {
    annyang.abort();  // برای اطمینان از اجرای مجدد صحیح
    ...
    const commands = {
      ...
      "restart": () => {
        this.subtitle = "Restarting the game!";
        this.restartGame();
      },
      "back to menu": () => {
        this.subtitle = "Returning to the menu!";
        this.goBackToMenu();
      },
      "select number *number": (number) => {
        this.subtitle = `Card number ${number} selected!`;
        this.selectCardByVoice(parseInt(number, 10));
      }
    };
